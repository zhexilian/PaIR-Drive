...
class TransfuserBackbone(nn.Module):
    ...
    def __init__(self, config: RWMConfig):

        super().__init__()
        self.config = config

        self.image_encoder = timm.create_model(config.image_architecture, pretrained=False, features_only=True)
        # The released RWM checkpoint contains the complete image encoder.  An
        # optional standalone ResNet initialization is supported for checkpoint
        # conversion/debugging, but is not needed for inference.
        if config.local_weight_path is not None:
            state_dict = torch.load(config.local_weight_path, map_location="cpu")
